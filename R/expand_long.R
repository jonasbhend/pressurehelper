...
#' @keywords util
#' @export
expand_long <- function(df, inventory=read_inventory()){
  ...
  for (nn in c('Year', 'Month', 'Day')) df[[nn]] <- as.numeric(df[[nn]])
  ...
  df$Time.missing[substr(df$Local.time, 1,2) != gsub('AM', '', gsub('PM', '', gsub(':.*', '', toupper(df$Time))))] <- 1
  ...
  datestring <- paste(apply(df[,c('Year', 'Month', 'Day')], 1, paste, collapse='-'), df$Local.time)
  ...
  for (nn in c('TA', 'TP')){
    ...
    if (length(Tnames) > 0){
      ...
      df[[paste0(nn, '.orig')]] <- temperature2SI(Temperature, units=T.units) - 273.15
    }    
  }
  ...
  if (P.unit == 'hPa'){
    ...
    if (P.unit == 'mm'){
      ...
      df$mmHg <- length2SI(as.matrix(df[,P.names]), units='', base=base[seq(P.names)]) * 1000
  ...
  if (is.null(df$Tcorr)){
    print('Assuming that pressure is not reduced to 0º C')
    df$Tcorr <- 0
  }
  if (df$Tcorr[1] == 1){
