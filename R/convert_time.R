...
convert_time <- function(year, month, day, time, latitude=NULL){
  ...
  numeric.i <- !is.na(timenum)
  if (sum(numeric.i) > 0){
    hours <- formatC(floor(timenum[numeric.i]), width=2, flag='0')
    minutes <- formatC(floor((timenum[numeric.i] %% 1)*60), width=2, flag='0')
    ## replace times with HH:MM
    time[numeric.i] <- paste(hours, minutes, sep=':')    
  }
